...
public class LineView extends View {
    ...
    private int YTextHeight = 0;
    private ArrayList<String> bottomTextList = new ArrayList<String>();
    private ArrayList<Integer> YTextList = new ArrayList<Integer>();
    ...
    public LineView(Context context, AttributeSet attrs) {
        ...
        YTextPaint.setTextSize(MyUtils.sp2px(getContext(), 12));
        YTextPaint.setTextAlign(Paint.Align.CENTER);
        YTextPaint.setStyle(Paint.Style.FILL);
        YTextPaint.setColor(BOTTOM_TEXT_COLOR);
    ...
    public void setDataList(ArrayList<ArrayList<Integer>> dataLists) {
        ...
        for (ArrayList<Integer> list : dataLists) {
            ...
            String longestStr = "";
            bottomTextDescent = 0;
            for (Integer S : YTextList) {
                String s = Integer.toString(S);
                YTextPaint.getTextBounds(s, 0, s.length(), r);
                if (YTextHeight < r.height()) {
                    YTextHeight = r.height();
                }
                if (autoSetGridWidth && (longestWidth < r.width())) {
                    longestWidth = r.width();
                    longestStr = s;
                }
                if (YTextDescent < (Math.abs(r.bottom))) {
                    YTextDescent = Math.abs(r.bottom);
                }
            }
